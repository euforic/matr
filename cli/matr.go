...
// Matr is the root command runtime.
type Matr struct {
...
// New creates a new Matr runtime.
func New() *Matr {
	return &Matr{
		tasks:    map[string]*Task{},
		aliases:  map[string]string{},
...
// SetOutputs overrides the help and error writers used by the runtime.
func (m *Matr) SetOutputs(helpOut, errorOut io.Writer) {
	if helpOut != nil {
		m.helpOut = helpOut
	}
	if errorOut != nil {
...
// Handle registers a new task handler with the runtime.
...
// OnExit executes a final function before the runtime exits.
